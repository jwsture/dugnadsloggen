...
export default function Dugnadsloggen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Sjekk om dette er Google-innlogging uten eksisterende profil
      const erGoogleInnlogging = session.user.app_metadata?.provider === "google";
      if (erGoogleInnlogging && naavaerende.length > 0) {
        // Det finnes andre medlemmer, men ingen match — gi beskjed
        setInfo(`Velkommen! Vi fant ingen eksisterende profil koblet til ${sesjonEpost}. En ny profil er opprettet. Ta kontakt med admin hvis du allerede er registrert som medlem, så kan de koble profilene.`);
      }
          ...
          {modus === "inn" && (
            <div style={{ display: "grid", gap: 14 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, margin: "4px 0" }}>
                <div style={{ flex: 1, height: 1, background: "rgba(200,186,160,0.4)" }} />
                <span style={{ fontSize: 12, color: C.dempet }}>eller</span>
                <div style={{ flex: 1, height: 1, background: "rgba(200,186,160,0.4)" }} />
              </div>
              <button
                style={{ width: "100%", background: "#fff", border: "1px solid #dadce0", borderRadius: 6, padding: "10px 16px", fontSize: 15, fontWeight: 500, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, color: "#3c4043" }}
                onClick={async () => {
                  setFeil("");
